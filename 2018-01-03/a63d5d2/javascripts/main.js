...
async function loadProjects() {
  const projects = await $.get("https://gci-server.herokuapp.com/api/repositories");
  projects.forEach((project) => {
    let html = "<div class='col-xs-12 col-sm-6 col-md-4 col-lg-3'>";
    html += "<div class='card'><div class='avatar'><img id='projectImage";
    html += project.name + "' alt='Project of JBoss'>";
    html += "<div class='social' style='font-size: 20px; margin-top: -35px;'>";
    html += "<b><p id='projectHeading" + project.name + "' style='font-size: 25px;'>";
    html += "</p>contributors</b><br><a target='_blank' href='//github.com/jboss-outreach/";
    html += project.name + "'><i class='fa fa-github' style='font-size: 30px; margin: 5px;''>";
    html += "</i></a></div><p id='projectTitle" + project.name;
    html += "' style='display: inline-block;'></p></div></div></div>";
    $("#repeatedCode").after(html);

    const escapedName = project.name.replace(/\./g, "\\\\.");
    if (project.name !== escapedName) {
      console.log(escapedName);
    }
    $("#projectImage" + escapedName).attr("src", project.image);
    $("#projectHeading" + escapedName).html(project.contributors);
    $("#projectTitle" + escapedName).html("&nbsp;" + project.name);
  });
}
loadProjects();

async function loadContributors() {
  const contributors = await $.get("https://gci-server.herokuapp.com/api/contributors");
  contributors.slice(0, 52).forEach((contributor) => {
    let html = "<div class='col-xs-12 col-sm-6 col-md-4 col-lg-3'><div class='card'>";
    html += "<div class='avatar'><img src=" + contributor.avatar;
    html += "><div class='contribs'><p style='margin-top: -2.5px !important;'>";
    html += contributor.contributions + " contribution";
    if (contributor.contributions > 1) {
      html += "s";
    }
    html += "</p><a href=" + contributor.profile + " class='contributor-gh'>";
    html += "<i class='fa fa-github fa-2x' aria-hidden='true'></i></a></div>";
