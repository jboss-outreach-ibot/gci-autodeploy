...
async function loadContributors() {
  ...
  for (let repo of repos) {
    ...
    let data = [];
    do {
      data = await $.ajax(repo.contributors_url + "?per_page=100&page=" + page);
  ...
  // Get the top 52 contributors by number of contributions
  allContributors = allContributors
    .sort((a, b) => b.contributions - a.contributions)
    .slice(0, 52);

  // Build the HTML for the top contributors
  allContributors.forEach((contributor) => {
    ...
    html += "<span>";
    html += contributor.login + "</span></div></div></div>";
